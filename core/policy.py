...
class LFUPolicy:
    ...
    def __init__(self, max_cache_size: int):
        ...
        self._max_size = max_cache_size
        self._cache = dict()
        logger.info(f"LFUPolicy initialized with max_cache_size={max_cache_size}")
    ...
    def admit(self, doc_id: str) -> None:
        ...
        if doc_id in self._cache:
            self._cache[str(doc_id)]+=1
            logger.debug(f"Document {doc_id} already in cache, frequency increased")
        else:
            # Add new document
            self._cache[doc_id] = 1
            logger.debug(f"Admitted document {doc_id} to cache")
    ...
    def evict_candidate(self) -> Optional[str]:
        ...
        if not self._cache:
            ...
            return None
        # The first item in OrderedDict is the least recently used
        lru_doc_id =min(self._cache, key=self._cache.get)
        logger.debug(f"Eviction candidate: {lru_doc_id}")
        return lru_doc_id
